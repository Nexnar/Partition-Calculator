...
public class Partition implements Comparable<Partition> {
  ...
  public int getNumAt(int index) {
    if (index < 0 || index >= numbers.size())
      throw new IllegalArgumentException(
          "Invalid index, must be in the interval " + "[0," + (numbers.size() - 1) + "].");
  ...
  /**
   * Checks whether the given partition has the same numbers as this partition. Based on the current
   * value of orderMatters, it will check accordingly.
   * 
   ...
   * @return true if obj is a Partition, has the same N, sum, and numbers (with respect to
   *         orderMatters), false otherwise
   * 
   */
  @Override
  ...
  /**
   * Compares Paritions based on their length. Partitions that have the same length will then
   * compare based on the contents of their number list. (e.g. if you have the list [2,3,1,1] and
   * [2,1,3,1], the latter would be considered smaller because 2 = 2 and 1 < 3) This is used
   * implicilty to sort ArrayLists of Partitions in the TesterUtility class. This assumes compared
   * Partitions have the same N value.
   * 
   * @param o the partition to compare to this one
   * @return 0 if they are equal (exactly the same) and , < 0 if this partition is smaller than o,
   *         and > 0 if this partition is greater
   */
  @Override
